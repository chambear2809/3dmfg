...
CONFIGMAP_NAME="${CONFIGMAP_NAME:-notification-service-bundle}"
SERVICE_ROOT="${SERVICE_ROOT:-${repo_root}/notification-service}"
...
kubectl -n "${NAMESPACE}" create configmap "${CONFIGMAP_NAME}" \
  ...
  --from-file=app_delivery_py="${SERVICE_ROOT}/app/delivery.py" \
...
app/delivery.py <= ${SERVICE_ROOT}/app/delivery.py
