...
CONFIGMAP_NAME="${CONFIGMAP_NAME:-order-ingest-bundle}"
SERVICE_ROOT="${SERVICE_ROOT:-${repo_root}/order-ingest}"
...
kubectl -n "${NAMESPACE}" create configmap "${CONFIGMAP_NAME}" \
  ...
  --from-file=app_parser_py="${SERVICE_ROOT}/app/parser.py" \
...
app/parser.py <= ${SERVICE_ROOT}/app/parser.py
EOF
